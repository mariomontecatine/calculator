...
buttonValues.forEach((val) => {
  ...
  if (val === "/") btn.setAttribute("id", "btnDivision");
  else if (val === "*") btn.setAttribute("id", "btnMultiply");
  else if (val === "+") btn.setAttribute("id", "btnAddition");
  else if (val === "=") btn.setAttribute("id", "btnEqual");
  else if (val === ".") btn.setAttribute("id", "btnDot");
  else if (val === "-") btn.setAttribute("id", "btnSubtract");
  else btn.setAttribute("id", `btn${val}`);
  btn.textContent = val;
  buttonsContainer.appendChild(btn);
});

function operate(type, num1, num2) {
  if (type === "+") solution = add(num1, num2);
  else if (type === "-") solution = subtract(num1, num2);
  else if (type === "*") solution = multiply(num1, num2);
  else solution = divide(num1, num2);
...
function handleInput(e) {
  ...
  if (numPressed === "=" || numPressed === "Enter") {
    ...
    if (number2 === "0" || String(solution) === number2) number2 = numPressed;
    else number2 += numPressed;
    displayNumbers(number2);
  }
}
...
function displayNumbers(num) {
  ...
  } else display.textContent = displayCurrentValue;
}
